...
Measurement only. Reuses consumed Phase 2S–3K DEV 14200–14699. No new seeds.
Does not change α, scaling math, `_hero_damage`, gates, defaults, or 2Q.

    python -m ml.fidelity_phase_3l
    python -m ml.fidelity_phase_3l --lobbies 8 --seed 14200 --non-evaluative
"""
...
DEFAULT_DIR = "results/sim_fidelity_phase_3l"
PHASE = "3L third-party elimination-chain attribution"
...
def run_phase_3l(
    *,
    lobbies: int = PHASE_3L_LOBBIES,
    seed: int = PHASE_3L_SEED,
    ...
    if abs(FROZEN_ALPHA - 0.5) > 1e-12:
        raise ValueError(f"Phase 3L must keep frozen α=0.5, got {FROZEN_ALPHA}")

    t0 = time.time()
    tag = "SMOKE (non-evaluative)" if non_evaluative else "DEV"
    print(
        f"[3L] {tag} greedy CONTROL — {lobbies} lobbies, "
    ...
    print("[3L] greedy TREATMENT (2Q + 2S) — same seeds", flush=True)
    ...
    decision = diagnose_phase_3l(chain, non_evaluative=non_evaluative)

    os.makedirs(out_dir, exist_ok=True)
    _write_json(os.path.join(out_dir, "decision.json"), decision)
    _write_json(
        os.path.join(out_dir, "attribution.json"),
        _slim_attr(chain.get("attribution")),
    )
    _write_json(
        os.path.join(out_dir, "very_late_attribution.json"),
        _slim_attr(chain.get("very_late_attribution")),
    )
    _write_json(
        os.path.join(out_dir, "timing_3k.json"),
        _slim_attr(chain.get("timing_3k")),
    )
    _write_json(
        os.path.join(out_dir, "matchmaking_3j.json"),
        _slim_attr(chain.get("matchmaking_3j")),
    )
    _write_json(
        os.path.join(out_dir, "pairing_3i.json"),
        _slim_attr(chain.get("pairing_3i")),
    )
    _write_json(
        os.path.join(out_dir, "leftover_3h.json"),
        chain.get("leftover_3h") or {},
    )
    _write_json(
        os.path.join(out_dir, "examples.json"),
        {
            "chain": (chain.get("attribution") or {}).get("examples") or {},
            "hp_gap": (chain.get("attribution") or {}).get("hp_examples") or {},
        },
    )
    _write_json(
        os.path.join(out_dir, "reconciliation.json"),
        chain.get("reconciliation") or {},
    )
    _write_json(
    ...
    contract["evaluation"] = {
        ...
        "base_seed": seed,
        "note": (
            "Phase 3L reuses consumed 2S–3K DEV 14200–14699. "
            "Measurement only; no new seeds. Do not rewrite 2Q."
        ),
        "non_evaluative": non_evaluative,
        "reused_phase_2s_dev": True,
        "stacked_on_phase_3k": True,
    }
    contract.update({
        "phase": PHASE,
        "phase_3l_methodology_version": METHODOLOGY_VERSION,
    ...
    report = {
        ...
        "decision": decision,
        "contract": contract,
        "attribution": _slim_attr(chain.get("attribution")),
        "very_late_attribution": _slim_attr(chain.get("very_late_attribution")),
        "timing_3k": _slim_attr(chain.get("timing_3k")),
        "matchmaking_3j": _slim_attr(chain.get("matchmaking_3j")),
        "pairing_3i": _slim_attr(chain.get("pairing_3i")),
    ...
    _write_json(os.path.join(out_dir, "phase_3l_report.json"), report)

    attr = chain.get("attribution") or {}
    rec = chain.get("reconciliation") or {}
    lock = chain.get("leftover_3h") or {}
    decomp = chain.get("decomposition_3g") or {}
    timing_3k = chain.get("timing_3k") or {}
    print(
        f"[3L] primary_finding={decision['primary_finding']} "
        f"earlier={attr.get('share_same_seat_earlier_elimination')} "
        f"cascade={attr.get('share_different_seat_alive_set_cascade')} "
        f"flip={attr.get('share_same_fight_outcome_flip')} "
        f"thresh={attr.get('share_same_outcome_damage_threshold')} "
        ...
        f"third_n={attr.get('n_third_party')} "
        f"third3k={timing_3k.get('attributed', timing_3k).get('ghost_bye_third_party') if isinstance(timing_3k.get('attributed', timing_3k), dict) else timing_3k.get('ghost_bye_third_party')} "
        ...
        f"row_elim_ok={rec.get('row_elim_ok')} "
        f"recon_ok={attr.get('reconciliation_ok')} "
        f"evaluative={decision.get('evaluative')}",
        flush=True,
    )
    print(f"[3L] wrote {out_dir}/ ({contract['runtime_sec']}s)", flush=True)
    return report
...
def main(argv: Optional[list] = None) -> int:
    ...
    run_phase_3l(
        lobbies=args.lobbies if args.lobbies is not None else PHASE_3L_LOBBIES,
        seed=args.seed if args.seed is not None else PHASE_3L_SEED,
