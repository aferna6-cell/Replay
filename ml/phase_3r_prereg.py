"""Phase 3R — post-play scale-sync input/timing attribution (measurement only).

Stacked on Phase 3Q (PR #63). Reuses consumed DEV 14200–14699. No new seeds.
...
remains reserved. Keep HOLD stack including #63.

For every T5/T6 3N class-(3) paired body, snapshots both arms immediately
after the last open-slot play and at every subsequent scale-sync through
combat start, then decomposes the published 3Q subsequent-scaling term
into:

* (1) different pre-sync target/input state
* (2) different number/timing/order of scale-sync applications
* (3) same inputs but different body allocation because membership differs
* (4) rounding / numeric residue
* (5) residual
"""
...
# Published 3Q locks (exact).
PHASE_3Q_PRIMARY_N_PAIRS = 4132
PHASE_3Q_PRIMARY_N_FIGHTS = 1033
PHASE_3Q_SHARE_SAME_STATE = 0.07190788470337045
PHASE_3Q_SHARE_LIFECYCLE = 0.4481470184535611
PHASE_3Q_SHARE_SCALING = 0.47994509684306846
PHASE_3Q_SHARE_RESIDUAL = 0.0
PHASE_3Q_SCALING_ABS_MASS = 12588.0
PHASE_3Q_TOTAL_ABS_MASS = 26228.0
PHASE_3Q_T1_SYNTH_CONTROL = PHASE_3P_T1_SYNTH_CONTROL
PHASE_3Q_T1_SYNTH_TREATMENT = PHASE_3P_T1_SYNTH_TREATMENT
PHASE_3Q_T3_SYNTH_CONTROL = PHASE_3P_T3_SYNTH_CONTROL
PHASE_3Q_T3_SYNTH_TREATMENT = PHASE_3P_T3_SYNTH_TREATMENT
...
NESTED_SCALE_SYNC_IDENTITY = (
    "subsequent_scaling = pre_sync_input_state + sync_timing_count + "
    "membership_allocation + rounding_residue + residual"
)

SCALE_FLOW_RECONCILE_IDENTITY = (
    "board increment = Σ body combat increments at each scale-sync; "
    "body increment from last-play synth to combat-start = Σ logged "
    "post-play scale-sync increments"
)

SAME_TURN_SYNC_IDENTITY = (
    "syncs are matched by turn after the last open-slot play; extra or "
    "missing turns are timing/count; common-turn residual_add×share "
    "splits input vs membership vs rounding"
)

NEXT_OBSERVABLE_DEFAULT = (
    "the largest T5/T6 post-play scale-sync term that is still "
    "observable on the subsequent-scaling identity, ranked before any "
    "behavior change"
)
...
def diagnose_phase_3r(
    comparison: Optional[Dict] = None,
    *,
    non_evaluative: bool = False,
) -> Dict:
    """Route the 3Q subsequent-scaling leftover to a scale-sync term."""
    out = {
        "methodology_version": METHODOLOGY_VERSION,
        "primary_finding": "implemented_not_evaluated",
        "feature_toggle": FEATURE_TOGGLE,
        "feature_toggle_default_off": True,
        "phase_3r_seed": PHASE_3R_SEED,
        "phase_3r_lobbies": PHASE_3R_LOBBIES,
        ...
        "phase_3q_share_scaling": PHASE_3Q_SHARE_SCALING,
    ...
    input_state = _f("pre_sync_input_state")
    timing = _f("sync_timing_count")
    membership = _f("membership_allocation")
    rounding = _f("rounding_residue")
    residual = _f("residual")
    ranked = _rank_parts([
        ("pre_sync_input_state", input_state),
        ("sync_timing_count", timing),
        ("membership_allocation", membership),
        ("rounding_residue", rounding),
        ("residual", residual),
    ])
    top = ranked[0]["component"] if ranked else "residual"
    first_field = (
        (comparison.get("attribution") or comparison).get(
            "modal_first_input_diverge_field"
        )
        or primary.get("modal_first_input_diverge_field")
    )
    ...
    if input_state is not None and input_state > SHARE_DOMINANT:
        finding = "pre_sync_input_state_dominates"
        field_note = (
            f" First-diverging upstream input is {first_field}."
            if first_field else
            " Trace which upstream input (Firestone / growth / recruit "
            "raw / entering pool / residual budget) first diverges."
        )
        next_step = (
            "Different pre-sync target/input state clears ~70% of the "
            "3Q subsequent-scaling leftover."
            + field_note
            + " Next hour: follow that first-diverging input upstream. "
            + _no_change_tail()
        )
    elif timing is not None and timing > SHARE_DOMINANT:
        finding = "sync_timing_count_dominates"
        next_step = (
            "Different number/timing/order of scale-sync applications "
            "clears ~70% of the 3Q subsequent-scaling leftover. Next "
            "hour: audit lifecycle timing fidelity of post-play "
            "scale-syncs. " + _no_change_tail()
        )
    elif membership is not None and membership > SHARE_DOMINANT:
        finding = "membership_allocation_dominates"
        next_step = (
            "Same scale-sync inputs allocate differently because board "
            "membership differs, clearing ~70% of the 3Q "
            "subsequent-scaling leftover. Next hour: route to the "
            "open-slot lifecycle lane. " + _no_change_tail()
        )
    else:
        finding = "ranked_residual_needs_next_observable"
        next_step = (
            "No input / timing / membership / rounding term clears ~70% "
            f"of the 3Q subsequent-scaling leftover (top={top}). Rank "
            f"components and pursue the largest residual observable: "
    ...
    out.update({
        ...
        "share_pre_sync_input_state": input_state,
        "share_sync_timing_count": timing,
        "share_membership_allocation": membership,
        "share_rounding_residue": rounding,
        "share_residual": residual,
        "modal_first_input_diverge_field": first_field,
        "primary": primary,
