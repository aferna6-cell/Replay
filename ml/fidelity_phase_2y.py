...
Measurement only. Reuses consumed Phase 2S–2X DEV 14200–14699. No new seeds.
Does not change α, scaling math, `_hero_damage`, gates, defaults, or 2Q.

    python -m ml.fidelity_phase_2y
    python -m ml.fidelity_phase_2y --lobbies 8 --seed 14200 --non-evaluative
"""
...
from ml.position_order_diagnostic import (
    ...
    run_greedy_control,
    summarize_position_arm,
)

DEFAULT_DIR = "results/sim_fidelity_phase_2y"
PHASE = "2Y slot/attack-order vs teammate protection"
...
def run_phase_2y(
    *,
    lobbies: int = PHASE_2Y_LOBBIES,
    seed: int = PHASE_2Y_SEED,
    ...
    if abs(FROZEN_ALPHA - 0.5) > 1e-12:
        raise ValueError(f"Phase 2Y must keep frozen α=0.5, got {FROZEN_ALPHA}")

    t0 = time.time()
    tag = "SMOKE (non-evaluative)" if non_evaluative else "DEV"
    print(
        f"[2Y] {tag} greedy CONTROL — {lobbies} lobbies, "
        f"seeds {seed}–{seed + lobbies - 1}",
        flush=True,
    )
    greedy_c = summarize_position_arm(run_greedy_control(lobbies, seed))
    print("[2Y] greedy TREATMENT (2Q + 2S) — same seeds", flush=True)
    greedy_t = summarize_position_arm(run_greedy_2s_treatment(lobbies, seed))
    greedy_cmp = compare_position(greedy_c, greedy_t)
    if non_evaluative:
        greedy_cmp["non_evaluative"] = True

    decision = diagnose_phase_2y(greedy_cmp, non_evaluative=non_evaluative)
    ...
    _write_json(
        os.path.join(out_dir, "by_tier_position.json"),
        greedy_cmp.get("by_tier_position") or {},
    )
    _write_json(
        os.path.join(out_dir, "survival_by_tier_slot_bin.json"),
        greedy_cmp.get("survival_by_tier_slot_bin") or {},
    )
    _write_json(
        os.path.join(out_dir, "survival_by_tier_teammate.json"),
        greedy_cmp.get("survival_by_tier_teammate_decile") or {},
    )
    _write_json(
    ...
    contract["evaluation"] = {
        ...
        "base_seed": seed,
        "note": (
            "Phase 2Y reuses consumed 2S–2X DEV 14200–14699. "
            "Measurement only; no new seeds. Do not rewrite 2Q."
        ),
        "non_evaluative": non_evaluative,
        "reused_phase_2s_dev": True,
        "stacked_on_phase_2x": True,
    }
    contract.update({
        "phase": PHASE,
        "phase_2y_methodology_version": METHODOLOGY_VERSION,
    ...
    report = {
        ...
        "by_tier_position": greedy_cmp.get("by_tier_position"),
    ...
    _write_json(os.path.join(out_dir, "phase_2y_report.json"), report)

    rw = greedy_cmp.get("reweighting") or {}
    print(
        f"[2Y] primary_finding={decision['primary_finding']} "
        f"resid={rw.get('phase_2x_residual_position_hat')} "
        f"slot={rw.get('share_of_residual_slot_opportunity')} "
        f"team={rw.get('share_of_residual_teammate_protection')} "
        f"unexpl={rw.get('share_of_residual_unexplained')} "
        f"evaluative={decision.get('evaluative')}",
        flush=True,
    )
    print(f"[2Y] wrote {out_dir}/ ({contract['runtime_sec']}s)", flush=True)
    return report
...
def main(argv: Optional[list] = None) -> int:
    ...
    run_phase_2y(
        lobbies=args.lobbies if args.lobbies is not None else PHASE_2Y_LOBBIES,
        seed=args.seed if args.seed is not None else PHASE_2Y_SEED,
