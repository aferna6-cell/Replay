...
Measurement only. Reuses consumed Phase 2S–3O DEV 14200–14699. No new seeds.
Does not change α, scaling math, `_hero_damage`, gates, defaults, or 2Q.

    python -m ml.fidelity_phase_3p
    python -m ml.fidelity_phase_3p --lobbies 8 --seed 14200 --non-evaluative
"""
...
DEFAULT_DIR = "results/sim_fidelity_phase_3p"
PHASE = "3P synthetic-pool allocation-input attribution"
...
def run_phase_3p(
    *,
    lobbies: int = PHASE_3P_LOBBIES,
    seed: int = PHASE_3P_SEED,
    ...
    print("[3P] reproducing 3O T5/T6 survivor-mechanic lock", flush=True)
    ...
    print("[3P] T5/T6 class-(3) allocation-input attribution", flush=True)
    ...
    _write_json(
        ...
        _slim_attr(allocation.get("matched_state")),
    )
    _write_json(
        os.path.join(out_dir, "mechanics_3o.json"),
        allocation.get("mechanics_3o") or {},
    )
    _write_json(
        os.path.join(out_dir, "examples.json"),
        {
            "t5_t6_class3": (
                (allocation.get("attribution") or {}).get("examples") or []
            ),
        },
    )
    _write_json(
        os.path.join(out_dir, "reconciliation.json"),
        allocation.get("reconciliation") or {},
    )
    _write_json(
        os.path.join(out_dir, "decomposition_3g.json"),
        allocation.get("decomposition_3g") or {},
    )
    ...
    contract["evaluation"] = {
        ...
        "base_seed": seed,
        "note": (
            "Phase 3P reuses consumed 2S–3O DEV 14200–14699. "
        ...
        "stacked_on_phase_3o": True,
    }
    contract.update({
        "phase": PHASE,
        "phase_3p_methodology_version": METHODOLOGY_VERSION,
    ...
    report = {
        ...
        "decision": decision,
        "contract": contract,
        "attribution": _slim_attr(allocation.get("attribution")),
        "primary": allocation.get("primary"),
        "per_tier": allocation.get("per_tier"),
        "source": allocation.get("source"),
        "matched_state_3n": _slim_attr(allocation.get("matched_state")),
        "mechanics_3o": allocation.get("mechanics_3o"),
        "reconciliation": allocation.get("reconciliation"),
        "decomposition_3g": allocation.get("decomposition_3g"),
    ...
    _write_json(os.path.join(out_dir, "phase_3p_report.json"), report)

    attr = allocation.get("attribution") or {}
    primary = allocation.get("primary") or {}
    rec = allocation.get("reconciliation") or {}
    print(
        f"[3P] primary_finding={decision['primary_finding']} "
        f"pool={primary.get('share_of_delta_pool_magnitude')} "
        f"weight={primary.get('share_of_delta_weight_composition')} "
        f"timing={primary.get('share_of_delta_timing_membership')} "
        f"rounding={primary.get('share_of_delta_integer_rounding')} "
        f"residual={primary.get('share_of_delta_residual')} "
        f"delta={primary.get('delta_synth')} "
    ...
    print(f"[3P] wrote {out_dir}/ ({contract['runtime_sec']}s)", flush=True)
    return report
...
def main(argv: Optional[list] = None) -> int:
    ...
    run_phase_3p(
        lobbies=args.lobbies if args.lobbies is not None else PHASE_3P_LOBBIES,
        seed=args.seed if args.seed is not None else PHASE_3P_SEED,
