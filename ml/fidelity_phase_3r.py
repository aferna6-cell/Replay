...
Measurement only. Reuses consumed Phase 2S–3Q DEV 14200–14699. No new seeds.
Does not change α, scaling math, `_hero_damage`, gates, defaults, or 2Q.

    python -m ml.fidelity_phase_3r
    python -m ml.fidelity_phase_3r --lobbies 8 --seed 14200 --non-evaluative
"""
...
DEFAULT_DIR = "results/sim_fidelity_phase_3r"
PHASE = "3R post-play scale-sync input/timing attribution"
...
def run_phase_3r(
    *,
    lobbies: int = PHASE_3R_LOBBIES,
    seed: int = PHASE_3R_SEED,
    ...
    print("[3R] reproducing 3O T5/T6 survivor-mechanic lock", flush=True)
    ...
    print("[3R] reproducing 3P allocation-input lock", flush=True)
    ...
    print("[3R] reproducing 3Q play-lifecycle lock", flush=True)
    ...
    print("[3R] T5/T6 class-(3) post-play scale-sync attribution", flush=True)
    ...
    decision = diagnose_phase_3r(scale, non_evaluative=non_evaluative)

    os.makedirs(out_dir, exist_ok=True)
    _write_json(os.path.join(out_dir, "decision.json"), decision)
    _write_json(
        os.path.join(out_dir, "attribution.json"),
        _slim_attr(scale.get("attribution")),
    )
    _write_json(
        os.path.join(out_dir, "primary.json"),
        scale.get("primary") or {},
    )
    _write_json(
        os.path.join(out_dir, "per_tier.json"),
        scale.get("per_tier") or {},
    )
    _write_json(
        os.path.join(out_dir, "syncs.json"),
        {
            "control": scale.get("syncs_control") or {},
            "treatment": scale.get("syncs_treatment") or {},
        },
    )
    _write_json(
    ...
    contract["evaluation"] = {
        ...
        "base_seed": seed,
        "note": (
            "Phase 3R reuses consumed 2S–3Q DEV 14200–14699. "
        ...
        "stacked_on_phase_3q": True,
    }
    contract.update({
        "phase": PHASE,
        "phase_3r_methodology_version": METHODOLOGY_VERSION,
    ...
    report = {
        ...
        "decision": decision,
        "contract": contract,
        "attribution": _slim_attr(scale.get("attribution")),
        "primary": scale.get("primary"),
        "per_tier": scale.get("per_tier"),
        "syncs_control": scale.get("syncs_control"),
        "syncs_treatment": scale.get("syncs_treatment"),
        "lifecycle_3q": scale.get("lifecycle_3q"),
        "source": scale.get("source"),
        "matched_state_3n": _slim_attr(scale.get("matched_state")),
    ...
    _write_json(os.path.join(out_dir, "phase_3r_report.json"), report)

    attr = scale.get("attribution") or {}
    primary = scale.get("primary") or {}
    rec = scale.get("reconciliation") or {}
    print(
        f"[3R] primary_finding={decision['primary_finding']} "
        f"input={primary.get('share_of_delta_pre_sync_input_state')} "
        f"timing={primary.get('share_of_delta_sync_timing_count')} "
        f"membership={primary.get('share_of_delta_membership_allocation')} "
        f"rounding={primary.get('share_of_delta_rounding_residue')} "
        f"residual={primary.get('share_of_delta_residual')} "
        f"scaling3q={attr.get('phase_3q_share_scaling')} "
        f"scaling_ok={attr.get('phase_3q_scaling_reproduced')} "
        ...
        f"nested_ok={rec.get('scale_nested_ok')} "
        f"flow_ok={rec.get('board_flow_ok')} "
        f"first_input={attr.get('modal_first_input_diverge_field')} "
        f"evaluative={decision.get('evaluative')}",
        flush=True,
    )
    print(f"[3R] wrote {out_dir}/ ({contract['runtime_sec']}s)", flush=True)
    return report
...
def main(argv: Optional[list] = None) -> int:
    ...
    run_phase_3r(
        lobbies=args.lobbies if args.lobbies is not None else PHASE_3R_LOBBIES,
        seed=args.seed if args.seed is not None else PHASE_3R_SEED,
