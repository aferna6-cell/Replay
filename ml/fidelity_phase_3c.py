...
Measurement only. Reuses consumed Phase 2S–3B DEV 14200–14699. No new seeds.
Does not change α, scaling math, `_hero_damage`, gates, defaults, or 2Q.

    python -m ml.fidelity_phase_3c
    python -m ml.fidelity_phase_3c --lobbies 8 --seed 14200 --non-evaluative
"""
...
DEFAULT_DIR = "results/sim_fidelity_phase_3c"
PHASE = "3C attacker-punch attribution"
...
def run_phase_3c(
    *,
    lobbies: int = PHASE_3C_LOBBIES,
    seed: int = PHASE_3C_SEED,
    ...
    if abs(FROZEN_ALPHA - 0.5) > 1e-12:
        raise ValueError(f"Phase 3C must keep frozen α=0.5, got {FROZEN_ALPHA}")

    t0 = time.time()
    tag = "SMOKE (non-evaluative)" if non_evaluative else "DEV"
    print(
        f"[3C] {tag} greedy CONTROL — {lobbies} lobbies, "
        f"seeds {seed}–{seed + lobbies - 1}",
        flush=True,
    )
    greedy_c = summarize_punch_arm(run_greedy_control(lobbies, seed))
    print("[3C] greedy TREATMENT (2Q + 2S) — same seeds", flush=True)
    greedy_t = summarize_punch_arm(run_greedy_2s_treatment(lobbies, seed))
    greedy_cmp = compare_punch(greedy_c, greedy_t)
    if non_evaluative:
        greedy_cmp["non_evaluative"] = True

    decision = diagnose_phase_3c(greedy_cmp, non_evaluative=non_evaluative)
    ...
    _write_json(
        os.path.join(out_dir, "by_tier_punch.json"),
        greedy_cmp.get("by_tier_punch") or {},
    )
    _write_json(
    ...
    contract["evaluation"] = {
        ...
        "base_seed": seed,
        "note": (
            "Phase 3C reuses consumed 2S–3B DEV 14200–14699. "
            "Measurement only; no new seeds. Do not rewrite 2Q."
        ),
        "non_evaluative": non_evaluative,
        "reused_phase_2s_dev": True,
        "stacked_on_phase_3b": True,
    }
    contract.update({
        "phase": PHASE,
        "phase_3c_methodology_version": METHODOLOGY_VERSION,
    ...
    report = {
        ...
        "by_tier_punch": greedy_cmp.get("by_tier_punch"),
    ...
    _write_json(os.path.join(out_dir, "phase_3c_report.json"), report)

    rw = greedy_cmp.get("reweighting") or {}
    rec = greedy_cmp.get("reconciliation") or {}
    print(
        f"[3C] primary_finding={decision['primary_finding']} "
        f"b3b={rec.get('reproduced_3b_damage_per_hit')} "
        f"atk={rw.get('share_of_b_attacker_attack_strength')} "
        f"synth={rw.get('share_of_b_attacker_synth_composition')} "
        f"pair={rw.get('share_of_b_pairing_order')} "
        f"unexpl={rw.get('share_of_b_still_unexplained')} "
        f"evaluative={decision.get('evaluative')}",
        flush=True,
    )
    print(f"[3C] wrote {out_dir}/ ({contract['runtime_sec']}s)", flush=True)
    return report
...
def main(argv: Optional[list] = None) -> int:
    ...
    run_phase_3c(
        lobbies=args.lobbies if args.lobbies is not None else PHASE_3C_LOBBIES,
        seed=args.seed if args.seed is not None else PHASE_3C_SEED,
