...
Measurement only. Reuses consumed Phase 2S–3N DEV 14200–14699. No new seeds.
Does not change α, scaling math, `_hero_damage`, gates, defaults, or 2Q.

    python -m ml.fidelity_phase_3o
    python -m ml.fidelity_phase_3o --lobbies 8 --seed 14200 --non-evaluative
"""
...
DEFAULT_DIR = "results/sim_fidelity_phase_3o"
PHASE = "3O matched-board survivor-mechanic attribution"
...
def run_phase_3o(
    *,
    lobbies: int = PHASE_3O_LOBBIES,
    seed: int = PHASE_3O_SEED,
    ...
    print("[3O] T5/T6 class-(3) survivor-mechanic attribution", flush=True)
    ...
    decision = diagnose_phase_3o(mechanics, non_evaluative=non_evaluative)

    os.makedirs(out_dir, exist_ok=True)
    _write_json(os.path.join(out_dir, "decision.json"), decision)
    _write_json(
        os.path.join(out_dir, "attribution.json"),
        _slim_attr(mechanics.get("attribution")),
    )
    _write_json(
        os.path.join(out_dir, "primary.json"),
        mechanics.get("primary") or {},
    )
    _write_json(
        os.path.join(out_dir, "full_class3.json"),
        mechanics.get("full_class3") or {},
    )
    _write_json(
        os.path.join(out_dir, "source.json"),
        mechanics.get("source") or {},
    )
    _write_json(
        os.path.join(out_dir, "matched_state_3n.json"),
        _slim_attr(mechanics.get("matched_state")),
    )
    _write_json(
        os.path.join(out_dir, "first_divergence_3m.json"),
        _slim_attr(mechanics.get("first_divergence_3m")),
    )
    _write_json(
        os.path.join(out_dir, "examples.json"),
        {
            "t5_t6_class3": (
                (mechanics.get("attribution") or {}).get("examples") or []
            ),
        },
    )
    _write_json(
        os.path.join(out_dir, "reconciliation.json"),
        mechanics.get("reconciliation") or {},
    )
    _write_json(
        os.path.join(out_dir, "decomposition_3g.json"),
        mechanics.get("decomposition_3g") or {},
    )
    ...
    contract["evaluation"] = {
        ...
        "base_seed": seed,
        "note": (
            "Phase 3O reuses consumed 2S–3N DEV 14200–14699. "
        ...
        "stacked_on_phase_3n": True,
    }
    contract.update({
        "phase": PHASE,
        "phase_3o_methodology_version": METHODOLOGY_VERSION,
    ...
    report = {
        ...
        "decision": decision,
        "contract": contract,
        "attribution": _slim_attr(mechanics.get("attribution")),
        "primary": mechanics.get("primary"),
        "full_class3": mechanics.get("full_class3"),
        "source": mechanics.get("source"),
        "matched_state_3n": _slim_attr(mechanics.get("matched_state")),
        "first_divergence_3m": _slim_attr(mechanics.get("first_divergence_3m")),
    ...
    _write_json(os.path.join(out_dir, "phase_3o_report.json"), report)

    attr = mechanics.get("attribution") or {}
    primary = mechanics.get("primary") or {}
    rec = mechanics.get("reconciliation") or {}
    print(
        f"[3O] primary_finding={decision['primary_finding']} "
        f"start_stats={primary.get('share_of_B_start_stats')} "
        f"attack={primary.get('share_of_B_attack_opportunity')} "
        f"target={primary.get('share_of_B_target_exposure')} "
        f"keywords={primary.get('share_of_B_represented_keywords')} "
        f"teammate={primary.get('share_of_B_teammate_protection')} "
        f"residual={primary.get('share_of_B_residual')} "
        f"B3n={attr.get('phase_3n_within_tier_B_hat')} "
        f"B3n_ok={attr.get('phase_3n_B_reproduced')} "
        f"class3_n={attr.get('n_same_outcome_damage')} "
        f"primary_n={attr.get('n_primary_class3')} "
        f"body_ok={rec.get('body_event_ok')} "
        f"nested_ok={rec.get('primary_nested_ok')} "
        f"evaluative={decision.get('evaluative')}",
        flush=True,
    )
    print(f"[3O] wrote {out_dir}/ ({contract['runtime_sec']}s)", flush=True)
    return report
...
def main(argv: Optional[list] = None) -> int:
    ...
    run_phase_3o(
        lobbies=args.lobbies if args.lobbies is not None else PHASE_3O_LOBBIES,
        seed=args.seed if args.seed is not None else PHASE_3O_SEED,
