"""Phase 3M — earliest same-seat HP divergence attribution (measurement only).

Stacked on Phase 3L (PR #58). Reuses consumed DEV 14200–14699. No new seeds.
...
reserved. Keep HOLD stack including #58.

Reproduces the 3L same-seat earlier-elimination class (2082 of 3701
third-party rows) and walks each causal seat backward from that earlier
elimination to the first turn where paired pre-combat or post-combat HP
diverges. Splits those 2082 rows exclusively into:

* (1) prior alive-set / pairing difference
* (2) same-pairing outcome flip
* (3) same-outcome damage magnitude / threshold
* (4) earlier already-diverged HP carry
...
FIRST_DIVERGENCE_RECONCILE_IDENTITY = (
    "prior_alive_set_or_pairing + same_pairing_outcome_flip + "
    "same_outcome_damage + inherited_hp_carry + unreconciled "
    "= 3L same_seat_earlier_elimination (2082)"
)

ROW_HISTORY_DIVERGENCE_IDENTITY = (
    "every class-(1) row maps to one causal-seat history and exactly "
    "one first-divergence event; that event's fight satisfies "
    "post_hp = pre_hp - applied_to_seat when a fight is present"
)

# Published 3D / 3E / 3F / 3G / 3H / 3I / 3J / 3K / 3L locks (exact).
...
PHASE_3L_SAME_SEAT_EARLIER = 2082
PHASE_3L_CASCADE = 561
PHASE_3L_OUTCOME_FLIP = 334
PHASE_3L_DAMAGE_THRESHOLD = 724
PHASE_3L_UNRECONCILED = 0
PHASE_3L_SHARE_EARLIER = 0.5625506619832478
PHASE_3L_PRIOR_HP = 1858
PHASE_3L_HIT = 120
PHASE_3L_MAG = 104
PHASE_3L_SHARE_PRIOR_HP = 0.8924111431316043
PHASE_3L_SHARE_HIT = 0.057637848222862634
PHASE_3L_SHARE_MAG = 0.04995100864553314
PHASE_3L_VERY_LATE_THIRD_PARTY = 506
PHASE_3L_VERY_LATE_EARLIER = 287
...
REPRESENTED_SOURCE = FIRST_DIVERGENCE_COMPONENTS

NEXT_OBSERVABLE_DEFAULT = (
    "the largest first-divergence class of the 3L same-seat earlier "
    "eliminations, ranked before any behavior change"
)
...
def assert_seed_range_allowed(seed: int, lobbies: int) -> None:
    """3M may only reuse 14200–14699. Confirm and all other consumed bands fail."""
    lo, hi = seed, seed + lobbies - 1
    if lo < REUSED_SEED_LO or hi > REUSED_SEED_HI:
        raise ValueError(
            f"Phase 3M must reuse {REUSED_SEED_LO}–{REUSED_SEED_HI}, "
            f"got {lo}–{hi}"
        )
    for flo, fhi in FORBIDDEN_RANGES:
        if lo <= fhi and hi >= flo:
            raise ValueError(
                f"Phase 3M seed range {lo}–{hi} overlaps forbidden {flo}–{fhi}"
            )
...
def share_of_class1(
    ...
    *,
    ...
    """Signed share of the 3L same-seat earlier class (2082 rows)."""
...
def classify_first_divergence(
    *,
    ...
    control_obs: bool = False,
    treatment_obs: bool = False,
    pre_hp_equal: bool = True,
    pairing_equal: bool = True,
    outcomes_equal: bool = True,
    applied_equal: bool = True,
    paired_fights_present: bool = True,
) -> str:
    """Exclusive class for one class-(1) first-divergence event.

    Walk the causal seat from the earliest recorded turn through the
    earlier elimination and stop at the first turn where paired
    pre-combat or post-combat HP diverges.

    Priority: missing event / missing arm observation → unreconciled;
    pre-combat HP already differs → inherited_hp_carry; pairing or
    alive-set differs → prior_alive_set_or_pairing; no paired fight at
    the first observed HP split → inherited_hp_carry; same pairing with
    outcome flip → same_pairing_outcome_flip; same pairing and same
    outcome but applied damage differs → same_outcome_damage;
    otherwise unreconciled.
    """
    if not found_event or not (control_obs and treatment_obs):
        return "unreconciled"
    if not pre_hp_equal:
        return "inherited_hp_carry"
    if not pairing_equal:
        return "prior_alive_set_or_pairing"
    if not paired_fights_present:
        return "inherited_hp_carry"
    if not outcomes_equal:
        return "same_pairing_outcome_flip"
    if not applied_equal:
        return "same_outcome_damage"
    return "unreconciled"


def diagnose_phase_3m(
    comparison: Optional[Dict] = None,
    *,
    non_evaluative: bool = False,
) -> Dict:
    """Route the 3L class-(1) leftover to a first-divergence class."""
    out = {
        "methodology_version": METHODOLOGY_VERSION,
        "primary_finding": "implemented_not_evaluated",
        "feature_toggle": FEATURE_TOGGLE,
        "feature_toggle_default_off": True,
        "phase_3m_seed": PHASE_3M_SEED,
        "phase_3m_lobbies": PHASE_3M_LOBBIES,
        ...
        "phase_3l_prior_hp": PHASE_3L_PRIOR_HP,
        "phase_3l_share_prior_hp": PHASE_3L_SHARE_PRIOR_HP,
    ...
    shares = {name: attr.get(f"share_{name}") for name in FIRST_DIVERGENCE_COMPONENTS}
    ranked = _rank_parts([(n, shares[n]) for n in FIRST_DIVERGENCE_COMPONENTS])
    ...
    pairing = _f("prior_alive_set_or_pairing")
    flip = _f("same_pairing_outcome_flip")
    damage = _f("same_outcome_damage")
    inherited = _f("inherited_hp_carry")
    unrec = _f("unreconciled")
    represented = [s for s in (pairing, flip, damage, inherited) if s is not None]
    ...
    if pairing is not None and pairing > SHARE_DOMINANT:
        primary = "prior_alive_set_or_pairing_dominates"
        next_step = (
            "Prior alive-set / pairing difference clears ~70% of the 3L "
            "same-seat earlier class. Next hour: recurse one "
            "matchmaking / elimination hop upstream. " + _no_change_tail()
        )
    elif flip is not None and flip > SHARE_DOMINANT:
        primary = "same_pairing_outcome_dominates"
        next_step = (
            "Same-pairing outcome flip clears ~70% of the 3L same-seat "
            "earlier class. Next hour: isolate combat-outcome fidelity. "
            + _no_change_tail()
        )
    elif damage is not None and damage > SHARE_DOMINANT:
        primary = "same_outcome_damage_dominates"
        next_step = (
            "Same-outcome damage magnitude / threshold clears ~70% of "
            "the 3L same-seat earlier class. Next hour: matched-state "
            "damage fidelity. " + _no_change_tail()
        )
    elif inherited is not None and inherited > SHARE_DOMINANT:
        primary = "inherited_hp_carry_dominates"
        next_step = (
            "Inherited HP carry clears ~70% of the 3L same-seat earlier "
            "class. Next hour: continue backward to the earliest "
            "originating fight rather than changing `_hero_damage`. "
            + _no_change_tail()
        )
    elif top_share >= 0.30:
        primary = "mixed_route_to_larger"
        next_step = (
            "No single first-divergence class clears ~70% of the 3L "
            f"same-seat earlier class (top={top}). Rank components and "
            "pursue the largest. " + _no_change_tail()
        )
    else:
        primary = "ranked_residual_needs_next_observable"
        next_step = (
            "No represented first-divergence class clears ~70% of the "
            f"3L same-seat earlier class (top={top}). Rank the residual "
    ...
    out.update({
        ...
        "share_prior_alive_set_or_pairing": pairing,
        "share_same_pairing_outcome_flip": flip,
        "share_same_outcome_damage": damage,
        "share_inherited_hp_carry": inherited,
