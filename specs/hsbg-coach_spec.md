...
| 14 | **Self-play policy** (`ml/econ_policy.py`) — learns to *act*, not just evaluate | ✅ PolicyNet (state → P(level/tempo)) trained by REINFORCE self-play (placement reward + baseline). Beats the heuristic field (vs-field avg ~3.1 < 4.5). Learned sensible strategy unsupervised: P(level) 36% when behind/under-tier → 3% when high-tier/ahead. Stress-tested the sim (its first pass exposed that the strength model under-rewarded tiering; fixed the tier dynamics, then it rediscovered correct play). Shown in `plan`; `train_policy.py`. |
