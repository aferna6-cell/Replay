...
| 12 | **Whole-game value** (`hsbg_coach/game_value.py`) — every move scored by expected FINAL placement | ✅ `expected_placement` blends the eval-net board value (learned, whole-game-trained) + multi-turn trajectory + HP risk; `rank_actions` ranks buy/sell/level/roll on one expected-finish axis. Wired into `advise` + the live overlay, so every suggested move accounts for the rest of the game. |
