...
| 10 | **Draft recommender** (`hsbg_coach/draft.py`) — rank an offered choice: hero / trinket / Discover | ✅ hero+trinket by meta avg-placement (trinkets board-fit-adjusted); Discover by board-fit (eval net + card2vec synergy on your live board). `hsbg pick <kind> ...` CLI. Live auto-detection of offers pending real-log calibration. |
| 11 | **Multi-turn planner** (`hsbg_coach/multiturn.py`) — look K turns ahead: tempo vs level vs greed | ✅ economy forward-model on the real pace curves; projects tier/board-vs-curve/HP under candidate strategies, scores each (terminal strength, tier, cumulative damage, death). Low-HP⇒tempo, under-tier⇒level, longer horizon⇒leveling pays. `hsbg plan` CLI. |
| 12 | **Whole-game value** (`hsbg_coach/game_value.py`) — every move scored by expected FINAL placement | ✅ `expected_placement` blends the eval-net board value (learned, whole-game-trained) + trajectory + HP risk; `rank_actions` ranks buy/sell/level/roll on one expected-finish axis. Wired into `advise` + the live overlay. |
| 13 | **Economy self-play sim** (`ml/econ_env.py`, `ml/econ_value.py`) — learned trajectory value | ✅ abstract 8-player economy lobby (gold/tier/strength-vs-curve growth + real damage/elimination) → placement-labeled trajectories → value net. First real piece of the self-play agent (the *environment*). Replaces the heuristic trajectory term in the whole-game value when trained. |
| 13b | **Calibration pass** | ✅ damage scales with board size → realistic alive-by-turn (8→~5 by t10→~2 by t14); broadened state coverage (exploration + variance) for off-policy reliability; 8-way placement-**distribution** head (proper-scored; exposes top-4 odds); realistic `players_left` via `alive_at(turn)`; reliability report in training. Verified well-calibrated marginally AND conditionally (turn-8 by-ratio \|err\| ≈ 0.27). Key insight: the pace curve is the *top-10%* benchmark, so "on-curve" ≈ winning. |
| 14 | **Self-play policy** (`ml/econ_policy.py`) — learns to *act*, not just evaluate | ✅ PolicyNet (state → P(level/tempo)) trained by REINFORCE self-play (placement reward + baseline + entropy). Beats the heuristic field. Learned the right *mindset* unsupervised — leveling aggression rises with board lead (snowball: 64%→94% behind→crushing at high HP) and falls with HP drain (ease off: 64%→22% high→crit HP). Shown in `plan`. Honest limit: its absolute pace is a touch conservative (plateaus ~tier 4 vs the ideal 5–6). |
| 14b | **Real tavern-tier curve** (`pace.STANDARD_TAVERN_TIER`) | ✅ replaced the mislabeled "avg tier of cards *played*" benchmark (plateaus ~3.6 — it's board composition, NOT tavern tier) with a real high-level tavern-tier-by-turn curve (tier 4 by t7, 5 by t9, 6 by ~t12). `pace_advice` + the econ sim's tier target + `hsbg pace` now use it, so leveling advice matches high-level lobbies. This is *why* pros are "ahead of the curve" — the old curve wasn't measuring tavern tier. Refine per-hero from real Power.logs (they log actual tavern tier each turn). |
