...
| 13 | **Economy self-play sim** (`ml/econ_env.py`, `ml/econ_value.py`) — learned trajectory value | ✅ abstract 8-player economy lobby (gold/tier/strength-vs-curve growth + real combat/damage/elimination) → placement-labeled trajectories → value net (val MAE 1.3, r 0.67 on held-out sims). Replaces the heuristic trajectory term in the whole-game value when trained. First real piece of the self-play agent (the *environment*). |
