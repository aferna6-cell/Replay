"""Phase 3L third-party elimination-chain attribution — observational locks."""
...
from ml.elimination_chain_diagnostic import (
    ...
    classify_third_party_chain as _cls_chain_imported,
    compare_chain,
...
def test_methodology_is_3l_v1_default_off():
    assert METHODOLOGY_VERSION == "3l_v1"
...
def test_reuses_2s_dev_and_forbids_confirm():
    assert PHASE_3L_SEED == 14200
    assert PHASE_3L_LOBBIES == 500
...
def test_hold_stack_includes_3k_and_frozen_alpha():
    assert HOLD_PRS == (
    ...
    assert d["row_elim_hp_identity"] == ROW_ELIM_HP_IDENTITY
    assert _cls_chain_imported is classify_third_party_chain
...
def test_share_and_classify_chain():
    assert abs(share_of_third_party(70.0, denom=100.0) - 0.70) < 1e-9
    assert share_of_third_party(None, denom=100.0) is None
    base = dict(
        control_present=True, treatment_present=True,
        causal_seat_control=2, causal_seat_treatment=2,
        same_fight_pairing=False, outcomes_equal=True,
        one_died_other_alive_at_elim_turn=True, applied_equal=True,
        elim_turns_equal=False,
    )
    assert classify_third_party_chain(**base) == "same_seat_earlier_elimination"
    assert classify_third_party_chain(
        **{**base, "causal_seat_control": 1, "causal_seat_treatment": 2}
    ) == "different_seat_alive_set_cascade"
    assert classify_third_party_chain(
        **{**base, "same_fight_pairing": True, "outcomes_equal": False}
    ) == "same_fight_outcome_flip"
    assert classify_third_party_chain(
        **{**base, "same_fight_pairing": True, "outcomes_equal": True,
           "applied_equal": False}
    ) == "same_outcome_damage_threshold"
    assert classify_third_party_chain(
    ...
    assert classify_third_party_chain(
        control_present=True, treatment_present=True,
        causal_seat_control=None, causal_seat_treatment=None,
    ) == "unreconciled"
    # Same-fight outcome beats earlier-elimination when both apply.
    assert classify_third_party_chain(
        **{**base, "same_fight_pairing": True, "outcomes_equal": False,
           "elim_turns_equal": False}
    ) == "same_fight_outcome_flip"
    # Different seats beat same-fight features.
    assert classify_third_party_chain(
        **{**base, "causal_seat_control": 1, "causal_seat_treatment": 2,
           "same_fight_pairing": True, "outcomes_equal": False}
    ) == "different_seat_alive_set_cascade"


def test_classify_chain_hp_gap_priority():
    base = dict(
        chain_class="same_seat_earlier_elimination",
        control_fight_present=True, treatment_fight_present=True,
        pre_hp_equal=True, control_hit=True, treatment_hit=True,
        applied_equal=True,
    )
    assert classify_chain_hp_gap(
    ...
    assert classify_chain_hp_gap(**base) == "hp_unreconciled"
    assert classify_chain_hp_gap(
        **{**base, "control_fight_present": False}
    ) == "hp_unreconciled"
    assert classify_chain_hp_gap(chain_class="same_fight_outcome_flip") is None
    assert classify_chain_hp_gap(
...
def test_diagnose_routes():
    prior = diagnose_phase_3l({
        "attribution": {
            "share_same_seat_earlier_elimination": 0.80,
            "share_different_seat_alive_set_cascade": 0.10,
            "share_same_fight_outcome_flip": 0.06,
            "share_same_outcome_damage_threshold": 0.03,
    ...
    assert prior["primary_finding"] == "same_seat_earlier_prior_hp_dominates"
    assert "HP paths separate" in prior["recommended_next_step"]
    hit = diagnose_phase_3l({
        "attribution": {
            "share_same_seat_earlier_elimination": 0.80,
            "share_different_seat_alive_set_cascade": 0.10,
            "share_same_fight_outcome_flip": 0.05,
            "share_same_outcome_damage_threshold": 0.04,
            "share_unreconciled": 0.01,
    ...
    assert hit["primary_finding"] == "same_seat_earlier_hit_dominates"
    assert "combat-outcome" in hit["recommended_next_step"]
    mag = diagnose_phase_3l({
        "attribution": {
            "share_same_seat_earlier_elimination": 0.80,
            "share_different_seat_alive_set_cascade": 0.10,
            "share_same_fight_outcome_flip": 0.05,
    ...
    assert mag["primary_finding"] == "same_seat_earlier_damage_dominates"
    assert "matched-state" in mag["recommended_next_step"]
    cascade = diagnose_phase_3l({
        "attribution": {
            "share_same_seat_earlier_elimination": 0.10,
            "share_different_seat_alive_set_cascade": 0.80,
            "share_same_fight_outcome_flip": 0.05,
            "share_same_outcome_damage_threshold": 0.04,
            "share_unreconciled": 0.01,
        }
    })
    assert cascade["primary_finding"] == "different_seat_cascade_dominates"
    assert "recursively" in cascade["recommended_next_step"]
    flip = diagnose_phase_3l({
        "attribution": {
            "share_same_seat_earlier_elimination": 0.10,
            "share_different_seat_alive_set_cascade": 0.08,
            "share_same_fight_outcome_flip": 0.80,
            "share_same_outcome_damage_threshold": 0.01,
            "share_unreconciled": 0.01,
        }
    })
    assert flip["primary_finding"] == "same_fight_outcome_dominates"
    assert "combat-outcome" in flip["recommended_next_step"]
    thresh = diagnose_phase_3l({
        "attribution": {
            "share_same_seat_earlier_elimination": 0.10,
            "share_different_seat_alive_set_cascade": 0.08,
            "share_same_fight_outcome_flip": 0.05,
            "share_same_outcome_damage_threshold": 0.76,
            "share_unreconciled": 0.01,
        }
    })
    assert thresh["primary_finding"] == "damage_threshold_dominates"
    assert "matched-state" in thresh["recommended_next_step"]
    mixed = diagnose_phase_3l({
        "attribution": {
            "share_same_seat_earlier_elimination": 0.40,
            "share_different_seat_alive_set_cascade": 0.25,
            "share_same_fight_outcome_flip": 0.20,
            "share_same_outcome_damage_threshold": 0.10,
            "share_unreconciled": 0.05,
        }
    })
    assert mixed["primary_finding"] == "mixed_route_to_larger"
    leftover_d = diagnose_phase_3l({
        "attribution": {
            "share_same_seat_earlier_elimination": 0.10,
            "share_different_seat_alive_set_cascade": 0.08,
            "share_same_fight_outcome_flip": 0.05,
    ...
    smoke = diagnose_phase_3l(mixed, non_evaluative=True)
...
def test_attribute_routes_same_seat_earlier_prior_hp():
    leftover = [_punch(14200, 0, t, loser=1) for t in LATE_TURNS] * 4
    # Leftover 0 vs 1 stays named. Third-party seat 2 dies T8 in treatment
    # vs seat 3 (low prior HP) and T11 in control vs seat 0 (different fight).
    c_alive = [0, 1, 2, 3]
    t_alive_late = [0, 1, 3]
    ...
    control = _arm_bundle(
        ...
        elims=[{"seed": 14200, "seat": 2, "turn": 11, "hp": 0, "survived": False}],
        hp_rows=[
    ...
    attr = attribute_elimination_chain(
        control, treatment, leftover_rows=leftover, treatment_punch=[],
    )
    assert attr["reconciliation_ok"] is True
    assert attr["hp_reconciliation_ok"] is True
    assert (attr["share_same_seat_earlier_elimination"] or 0.0) > 0.70
    assert (attr["share_accumulated_prior_hp"] or 0.0) > 0.70
    examples = attr["examples"]["same_seat_earlier_elimination"]
    assert examples
    rec = examples[0]
    assert rec["causal_seat"] == 2
    assert rec["treatment_elimination_turn"] == 8
    assert rec["control_elimination_turn"] == 11
    assert rec["treatment_decisive"]["pre_combat_hp"] == 3
    assert rec["control_decisive"]["opponent"] == 0 or rec["hp_gap_class"] == "accumulated_prior_hp"
    assert rec["treatment_decisive"]["prior_turn_hp"] == 3
    assert rec["control_alive_at_elim_turn"] is True
    assert rec["treatment_eliminated_at_elim_turn"] is True
    decision = diagnose_phase_3l({"attribution": attr})
    assert decision["primary_finding"] == "same_seat_earlier_prior_hp_dominates"


def test_attribute_routes_different_seat_cascade():
    leftover = [_punch(14200, 0, t, loser=3) for t in LATE_TURNS] * 4
    # Named leftover 0 + opponent 3. Control loses third-party seat 1 at T8;
    # treatment loses third-party seat 2 at T8.
    c_alive_late = [0, 2, 3]
    t_alive_late = [0, 1, 3]
    early = [0, 1, 2, 3]
    c_decs = (
        [_decision(14200, t, early, [(0, 3), (1, 2)]) for t in (7, 8)]
        + [_decision(14200, t, c_alive_late, [(0, 3), (2, None)], dead=[1, 4, 5, 6, 7])
           for t in (9, 10, 11, 12, 13, 14)]
    )
    t_decs = (
        [_decision(14200, t, early, [(0, 3), (1, 2)]) for t in (7, 8)]
        + [_decision(14200, t, t_alive_late, [(0, 3), (1, None)], dead=[2, 4, 5, 6, 7])
           for t in (9, 10, 11, 12, 13, 14)]
    )
    c_fights = [_fight(14200, t, 0, 3) for t in LATE_TURNS]
    c_fights.append(_fight(14200, 8, 2, 1, pre_b=4, applied=6, post_b=-2))
    t_fights = _ghost_fights(14200, 0)
    t_fights.append(_fight(14200, 8, 1, 2, pre_b=5, applied=6, post_b=-1))
    control = _arm_bundle(
        14200, opp=3, alive=c_alive_late, pairs=[(0, 3), (2, None)],
        dead=[1, 4, 5, 6, 7], fights=c_fights,
        elims=[{"seed": 14200, "seat": 1, "turn": 8, "hp": 0, "survived": False}],
    )
    control["pairing_decisions"] = c_decs
    treatment = _arm_bundle(
        14200, opp=3, alive=t_alive_late, pairs=[(0, 3), (1, None)],
        dead=[2, 4, 5, 6, 7], fights=t_fights,
        elims=[{"seed": 14200, "seat": 2, "turn": 8, "hp": 0, "survived": False}],
    )
    treatment["pairing_decisions"] = t_decs
    ...
    assert attr["reconciliation_ok"] is True
    assert (attr["share_different_seat_alive_set_cascade"] or 0.0) > 0.70
    examples = attr["examples"]["different_seat_alive_set_cascade"]
    assert examples
    rec = examples[0]
    assert rec["control_causal_seat"] == 1
    assert rec["treatment_causal_seat"] == 2
    assert rec["decisive_fight_turn"] == 8
    decision = diagnose_phase_3l({"attribution": attr})
    assert decision["primary_finding"] == "different_seat_cascade_dominates"


def test_attribute_routes_same_fight_outcome_flip():
    leftover = [_punch(14200, 0, t, loser=1) for t in LATE_TURNS] * 4
    c_alive = [0, 1, 2, 3]
    t_alive_late = [0, 1, 3]
    c_decs = [_decision(14200, t, c_alive, [(0, 1), (2, 3)]) for t in INSTRUMENT_TURNS]
    ...
    c_fights = [_fight(14200, t, 0, 1) for t in LATE_TURNS]
    # Same pairing 2 vs 3 at T8; control seat 2 wins, treatment seat 2 loses.
    c_fights.append(_fight(14200, 8, 2, 3, pre_a=10, pre_b=10, applied=4, post_a=10, post_b=6))
    t_fights = _ghost_fights(14200, 0)
    t_fights.append(_fight(14200, 8, 3, 2, pre_a=10, pre_b=10, applied=10, post_a=10, post_b=0))
    ...
    assert attr["reconciliation_ok"] is True
    assert (attr["share_same_fight_outcome_flip"] or 0.0) > 0.70
    examples = attr["examples"]["same_fight_outcome_flip"]
    assert examples
    rec = examples[0]
    assert rec["causal_seat"] == 2
    ...
    assert rec["treatment_decisive"]["applied_damage"] == 10
    assert rec["treatment_decisive"]["survivor_count"] == 2
    assert rec["treatment_decisive"]["tavern_tier"] == 5
    decision = diagnose_phase_3l({"attribution": attr})
    assert decision["primary_finding"] == "same_fight_outcome_dominates"


def test_attribute_routes_damage_threshold():
    leftover = [_punch(14200, 0, t, loser=1) for t in LATE_TURNS] * 4
    c_alive = [0, 1, 2, 3]
    t_alive_late = [0, 1, 3]
    c_decs = [_decision(14200, t, c_alive, [(0, 1), (2, 3)]) for t in INSTRUMENT_TURNS]
    ...
    c_fights = [_fight(14200, t, 0, 1) for t in LATE_TURNS]
    # Same pairing, both lose; treatment damage is lethal.
    c_fights.append(_fight(14200, 8, 3, 2, pre_a=20, pre_b=10, applied=4, post_b=6))
    t_fights = _ghost_fights(14200, 0)
    t_fights.append(_fight(14200, 8, 3, 2, pre_a=20, pre_b=10, applied=12, post_b=-2))
    control = _arm_bundle(14200, fights=c_fights, elims=[])
    control["pairing_decisions"] = c_decs
    treatment = _arm_bundle(
        14200, alive=t_alive_late, pairs=[(0, 1), (3, None)],
    ...
    assert attr["reconciliation_ok"] is True
    assert (attr["share_same_outcome_damage_threshold"] or 0.0) > 0.70
    examples = attr["examples"]["same_outcome_damage_threshold"]
    assert examples
    rec = examples[0]
    assert rec["causal_seat"] == 2
    assert rec["same_fight_pairing"] is True
    ...
    assert rec["treatment_applied"] == 12
    decision = diagnose_phase_3l({"attribution": attr})
    assert decision["primary_finding"] == "damage_threshold_dominates"
...
def test_compare_chain_reproduces_3g_on_synthetic():
    ...
    cmp = compare_chain(control, treatment, timing=timing)
