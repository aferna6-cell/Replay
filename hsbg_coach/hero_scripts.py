...
_HAND_ITEM_IDS = {
    ...
    "BG35_MagicItem_817t": "Duplicating Lens",  # token generated by Lens Case
}
_HAND_ITEM_NAMES = {v.lower(): k for k, v in _HAND_ITEM_IDS.items()}
_HAND_ITEM_NAME_NEEDLES = ("lens case", "duplicating lens")
...
def hand_item_play_lines(snapshot) -> List[str]:
    ...
    for m in hand:
        ...
        nm = _name(m)
        low = nm.lower()
        if "duplicating lens" in low:
            out.append(
                f"Play {nm} from hand — Lens Case token, play it now (hero script)"
            )
        elif "lens case" in low:
            out.append(
...
def gallywix_buy_adjust(action, snapshot, kb=None) -> Tuple[float, Optional[str]]:
    ...
    if gold < 3:
        return 0.0, None
    return -0.45, "Gallywix — cycle: buy to churn gold into HP value"
...
def gallywix_roll_adjust(snapshot, kb=None) -> Tuple[float, Optional[str]]:
    ...
    shop = list(_get(snapshot, "shop", []) or [])
    if gold >= 3 and shop:
        return 0.70, "Gallywix — don't roll past cycle fuel"
    return 0.0, None
