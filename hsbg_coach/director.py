...
_GAME_RULES_ESSENTIALS = (
    ...
    "Tavern SPELLS in the shop (their text is shown) are real purchases — "
    "weigh them like minions; a strong spell (e.g. discover/economy) often "
    "beats a mediocre minion. PURCHASE ORDER matters: when you plan several "
    "buys this turn, buy gold/coin-GENERATING cards first (a Battlecry that "
    "gives a Coin pays for the next buy — the order can be the whole line). "
    "FREEZE discipline: freeze is rare — only when the shop holds something "
    "genuinely worth keeping (a triple-completer, a core comp piece) that "
    "you cannot afford this turn. Never freeze a mediocre shop just because "
    "gold is spent. "
    "EFFECTS BEAT STATS: from the midgame on, a minion's effect text is "
    "usually worth more than its statline — economy generators, tribe "
    "buffers, triple pieces, and scaling engines outrank a vanilla bigger "
    "body. The engine's numbers lean stats-heavy; correct for that. Raw "
    "stats win only when you must survive the next combat. "
    "ACTIVATABLES: every turn, check the state line for a usable hero "
    "power or dark gift — using them at the right time is part of the "
    "optimal line, and a free/cheap activation is almost never wrong to "
    "weigh. "
