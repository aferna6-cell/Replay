"""DEV evaluation and policy drift for Experiment 5 dose-response runs."""
...
def main() -> int:
    ...
    kl_map = {"beta001": 0.01, "beta003": 0.03}
    jobs = []
    for kl_label in NEW_LABELS:
        kl_coef = kl_map[kl_label]
        for seed in SEEDS:
            run = os.path.join(BASE_DIR, kl_label, f"seed_{seed}")
    ...
    print(f"Total DEV eval jobs (new arms only): {len(jobs)}")
    t0 = time.time()
    with ProcessPoolExecutor(max_workers=8) as pool:
        eval_results = list(pool.map(run_eval, jobs))

    drift_jobs = [(kl, s) for kl in NEW_LABELS for s in SEEDS]
    with ProcessPoolExecutor(max_workers=8) as pool:
