...
RULES = [
    ...
    (r"\bi(?:'m| am|'?ll|(?:\s+am)?\s+gonna)?\s+buy(?:ing)?\b|\bbought\s+(?:it|this|that|the)\b|\bbuy\s+(?:this|that|it|like|the|a)\b|\byou\s+buy\s+the\b|\btake\s+(?:this|that|it)\b", "buy", "shop_buy", 0.7),
    (r"press(?:ing)?\s+(?:the\s+)?dark\s+gift|dark\s+gift\s+again|lost\s+the\s+gift", "discover", "dark_gift", 0.8),
    (r"\bcould\s+(?:have\s+)?sold|\bsell\s+(?:this|that|you|trash|my)\b|\bsold\s+(?:this|that|trash)\b|\bi(?:'?ll)?\s+sell\b", "sell", "sell_chaff", 0.65),
