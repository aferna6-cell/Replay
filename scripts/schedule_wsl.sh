...
if [ "${1:-}" = "--remove" ]; then
  "$SCHTASKS" /Delete /TN "$TASK_HS" /F 2>/dev/null || true
  "$SCHTASKS" /Delete /TN "$TASK_LOGS" /F 2>/dev/null || true
  echo "Removed both Windows scheduled tasks."
  exit 0
fi
if [ "${1:-}" = "--status" ]; then
  "$SCHTASKS" /Query /TN "$TASK_HS" 2>/dev/null || echo "$TASK_HS: not installed"
  "$SCHTASKS" /Query /TN "$TASK_LOGS" 2>/dev/null || echo "$TASK_LOGS: not installed"
  exit 0
fi
